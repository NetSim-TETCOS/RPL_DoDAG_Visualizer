...
wireshark_install_path = 'C:\Program Files\Wireshark'
ip_addr = []
node_x = []
node_y = []
color_map = []
        ...
        for i, line in enumerate(open('configuration.netsim')):
            try:
                if l_flag==0:
                    ...
                    found=re.search("<PROTOCOL_PROPERTY DEFAULT_GATEWAY=\"(.+?)\" IP_ADDRESS=\"(.+?)\"",line).group(1,2) 
                    ip_addr.append(found[1])
...
def get_rank_from_pcap(filename,flag):
    rank=""
    command = "\""+wireshark_install_path+"\\tshark.exe\""+ " -r "+ filename +" -T fields -R icmpv6.code==1 -Y ipv6.src=="+ip_addr[-1]+" -e icmpv6.rpl.dio.rank -2 >rank.txt"
    os.system(command)
...
iter_csv = pd.read_csv(tracepath, usecols=["CONTROL_PACKET_TYPE/APP_NAME","TRANSMITTER_ID","RECEIVER_ID"], iterator=True, chunksize=1000)
